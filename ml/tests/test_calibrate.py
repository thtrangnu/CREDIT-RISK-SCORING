...
def miscalibrated_data():
    """Score bị nén (systematically underconfident) so với xác suất thật -> ECE cao."""
    rng = np.random.default_rng(42)
    n = 4000
    true_p = rng.uniform(0.05, 0.95, size=n)
    y = rng.binomial(1, true_p)
    raw_score = np.clip(true_p * 0.5, 0.0, 1.0)  # nén về gần 0 -> miscalibrated
    return raw_score, y
...
def test_nested_calibrate_never_lets_a_fold_see_its_own_label(miscalibrated_data):
    """Sanity check gián tiếp: nested calibrate trên data ngẫu nhiên (không tín hiệu)
    không được cho ECE ảo thấp bất thường do leakage — so với calibrate 1 lần trên toàn bộ."""
    score, y = miscalibrated_data
    ...
    # nested (out-of-fold, trung thực) không được tốt hơn hẳn naive (in-sample, lạc quan)
